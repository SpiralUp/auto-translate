...
function initTranslator(translatorConfig) {
    ...
    if (translatorProvider === 'google') {
        ...
        googleTranslateClient = new Translate({ key: googleTranslateKey });
    }
}
...
function findInDictionary(text, fromLang, toLang) {
    ...
    if (useProjectDict && _projectDictIsOpen) {
        if (!_.isUndefined(projectDict[langKey]) && !_.isUndefined(projectDict[langKey][text])) {
            translation = projectDict[langKey][text];
            foundInProjectDict = true;
            return { translation, foundInProjectDict, foundInGlobalDict };
        }
    }

    if (_dictionaryIsOpen && !_.isUndefined(dictionary[langKey]) && !_.isUndefined(dictionary[langKey][text])) {
        translation = dictionary[langKey][text];
        foundInGlobalDict = true;

        // If not found in project dictionary, add it
        if (useProjectDict && !foundInProjectDict) {
            addToProjectDictionary(langKey, text, translation);
        }
 ...
 */
function addToProjectDictionary(langKey, text, translation) {
    if (useProjectDict) {
 ...
 * @param toLang
 */
function addToDictionary(text, translation, fromLang, toLang, addToGlobal = true, addToProject = true) {
    const langKey = `${fromLang}_${toLang}`;
...
function translateText(key, textToTranslate, fromLang, toLang) {
    return new Promise(function (resolve, reject) {
        const { translation: t, foundInProjectDict, foundInGlobalDict } = findInDictionary(key, fromLang, toLang);
        ...
        if (!t) {
            if (isAutomaticTranslation()) {
                ...
                const handleTranslationResult = translation => {
                    addToDictionary(key, translation, fromLang, toLang, addToGlobal, addToProject);
                    resolve(translation);
                };
